...
do
    temp=${file%.csv}
    address=${temp##*/}
    for block in `sort -rt, -k2 -n data-scripts/active-region/$exchange_name/txcount_$address.csv | grep ,[0-9]$ | head -n 30 | cut -f1 -d,`
    #for block in `sort -rt, -k2 -n data-scripts/active-region/$exchange_name/txcount_$address.csv | head -n 30 | cut -f1 -d,`
    do
